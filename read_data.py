...
def get_instrucion(task_name):
    ...
    ### tasks with line index ###
    # still need more log for whole function?
    if task_name == "task21":
        prefix = "'<line#>' in the code indicate the line ID at each newline position. "
        instructions = [
            "Please assess this code and determine whether more logging is required.",
            "Evaluate this piece of code to ascertain if additional logging is necessary.",
            "Analyze this code snippet and identify if further logging is needed.",
    ...
    if task_name == "task22":
        prefix = "The input consists of the code (after '<CODE>', with each line separated by '<line#>') and the line index to be inferred (after '<LINE>'). "
        instructions = [
            ...
            "Review the following code snippet and decide if it needs logging for the given line.",
            ...
            "Please assess the code and decide if it requires additional logging for the given line.",
            "Check the code and confirm whether it needs logging for the line.",
            "Study the provided code and specify if it could use more logging for the given line."
        ]
        suffix = " The expected output should be 'Yes' or 'No'."
        instruction = prefix + random.choice(instructions) + suffix
    ...
    # predict log level
    if task_name == "task25":
        prefix = "Log levels typically include 'fatal','error', 'warn', 'info', 'debug', and 'trace'. '<line#>' in the code indicate the line ID at each newline position. "
        instructions = [
            "In this function, please replace each 'UNKNOWN' log level with the corresponding log level based on its severity.",
            "Assess the function and allocate correct log levels to the log statements.",
            "Please review the function, then determine and assign the appropriate log levels to each log statement.",
    ...
    if task_name == "task29":
        prefix = "'<line#>' in the code indicate the line ID at each newline position. "
        instructions = [
            "Identify where in the code log entries should be added. Specify their log levels and the corresponding text messages.",
            "Suggest the suitable positions in the code for log statements. Indicate their logging levels and create appropriate messages.",
            "Determine where to insert log lines in the code. Define their levels and craft related messages.",
            "Go over the code, pinpoint locations for log entries, label their log levels and form associated messages.",
            ...
            "Analyze the code attentively, spotlight optimal locations for log entries, assign their severity levels, and generate fitting messages.",
        ...
        suffix = ' The expected output should be the log statements needed, such as "<line#> log.error(e)".'
        instruction = prefix + random.choice(instructions) + suffix
    # output fuction with added logging statement
    if task_name == "task30":
        prefix = "'<line#>' in the code indicate the line ID at each newline position. "
        instructions = [
            "Incorporate important log entry into this function to keep track of its operation.",
            "Execute log statement at critical point in the function to keep track of its progress.",
            "Add meaningful log in this function for potential debugging or execution tracing.",
            ...
            "Generate log for this function to record important actions or variable values during its execution.",
            "Please embed appropriate logging language within this function to aid in monitoring its running state.",
    ...
    # input seperate log and code, output code with inserted log
    if task_name == "task31": #这个任务有没有必要？插入之后输出有行号的function？感觉有点多余
        prefix = "The input are several log statements (post '<LOG>', split by '<DIV>') and a function (post '<CODE>', with each line separated by '<line#>'). "
        instructions = [
            "Incorporate the given log statements into the function at appropriate positions for best readability and debugging effectiveness.",
            "Adjust the structure of the function by positioning the provided log statements at their appropriate locations.",
            "Based on the given log data, identify the best locations within the function to incorporate each log statement.",
